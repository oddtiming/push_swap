...
bool vector_init(t_vector *vector)
{
	...
	vector->list.capacity = VECTOR_INIT_SIZE;
	vector->list.elems = malloc(VECTOR_INIT_SIZE * sizeof(int *));
	if (!vector->list.elems)
		return (FAILURE);
	return (SUCCESS);
}
...
bool vec_resize(t_vector *vector, int new_size)
{
	...
	if (new_size < vector->list.capacity)
		return (FAILURE);
	if (DEBUG)
	...
	else
	{
		...
		free (vector->list.elems);
		vector->list.elems = temp;
		if (DEBUG)
			printf(RED"SURVIVED FREE()\n"RESET_COL);
		vector->list.capacity = new_size;
		status = SUCCESS;
	}
	return (status);
}
